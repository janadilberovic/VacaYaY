...
1. **Scope the diff.** From the changed files above, split the diff into a **backend** slice
   (paths under `src/`) and a **frontend** slice (paths under `web/`). Ignore tooling-only
   changes (`.claude/`, config) unless the user asked about them.

2. **Dispatch to the specialists that apply** — and when both areas changed, launch them **in
