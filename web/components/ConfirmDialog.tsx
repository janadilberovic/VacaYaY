...
export function ConfirmDialog({ spec, onCancel }: { spec: ConfirmSpec; onCancel: () => void }) {
  return (
    <div className="modal-backdrop center" style={{ zIndex: 60 }}>
      <div className="modal-card" style={{ width: '100%', maxWidth: 400, padding: '22px 24px' }}>
        <div style={{ fontSize: 15.5, fontWeight: 700, marginBottom: 6 }}>{spec.title}</div>
        <div style={{ color: 'var(--text2)', fontSize: 13.5, marginBottom: 20 }}>
          {spec.message}
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button
            className="btn"
            style={{
              background: 'var(--pill-rejected-fg)',
              color: '#fff',
              padding: '8px 16px',
              fontSize: 13,
            }}
